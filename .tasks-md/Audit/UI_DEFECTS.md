...
|----|-----|---------|------------|------|
| D-101 | Steps | Layout still uses legacy `.ide-step-row`/`.ide-badge` classes; does not match design's new Steps grid with reorder/duplicate/dependency-warning rows | `frontend/src/v4/secondary-tabs.jsx::StepsTab` is the pre-design version | Port design `yui/project/secondary-tabs.jsx::StepsTab` (Manual Builder + dropdown menus + dependency warnings + locator chips) when backend events for those features land |
