...
| Step blocked action | `step-blocked-action-${stepId}` | `secondary-tabs.jsx::StepBlockedStrip` | `secondary-tabs.test.jsx` | — | ACTIVE | Pass 4b-4.1 DONE. Button enabled; dispatches typed command per `blocked.reason` via `_blockedActionCommand`: `missing_data→correction`, `permission_required→permission_decision`, `locator_unstable→improve_locator`, `wrong_page→navigate_to_expected`, `unknown→skip_step`. |
...
| Step precondition action | `step-precondition-action-${stepId}` | `secondary-tabs.jsx::StepPreconditionStrip` | `secondary-tabs.test.jsx` | — | ACTIVE | Pass 4b-5.1 DONE. Button enabled; dispatches `{ type: "change_precondition", step_id, expected_url }` via `onChangePrecondition`. |
| Step navigate to expected | `step-navigate-expected-${stepId}` | `secondary-tabs.jsx::StepPreconditionStrip` | `secondary-tabs.test.jsx` | — | ACTIVE | Pass 4b-5.1 NEW. Renders only when `precondition.status === "failed"` AND `expected_url` is non-empty string. Dispatches `{ type: "navigate_to_expected", step_id, expected_url }` via `onNavigateToExpected`. |
...
| Step improve-locator button | `step-improve-locator-${stepId}` | `secondary-tabs.jsx::StepLocatorChip` | `secondary-tabs.test.jsx` (D-101, 5 tests) | — | ACTIVE | D-101. Renders inside StepLocatorChip when `locator_kind ∈ {warn, med, unknown}` and step has id. Disabled when no `onImproveLocator` handler wired. Click dispatches `{type:"improve_locator", step_id}`. Backend seam: `server.py` → `improve_locator_acknowledged`. |
| Step view-candidates button | `step-view-candidates-${stepId}` | `secondary-tabs.jsx::StepLocatorChip` | `secondary-tabs.test.jsx` (D-101, 3 tests) | — | ACTIVE | D-101. Co-rendered with improve-locator button. Disabled when no `onViewCandidates` handler. Click dispatches `{type:"view_candidates", step_id}`. Backend maps to same seam as improve_locator. |
| Step change-locator-scope button (inline) | `step-change-locator-scope-${stepId}` | — | `secondary-tabs.jsx::StepLocatorChip` | — | DISABLE_WITH_REASON | D-101. Inline per-step scope selector absent from StepLocatorChip (requires scope-text UI not yet designed). Card-level `locator-change-scope` in CardLocatorAmbiguity remains wired via dispatcher. Sprint 8. |
