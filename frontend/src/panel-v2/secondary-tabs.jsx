...
import { PortalMenu } from './chrome.jsx';

function ManualBuilder() {
  const [action, setAction] = useState("click");
  const [assertion, setAssertion] = useState("visible");
...
export function StepsTab({ mode = "llm", setMode = () => {} }) {
  const [q, setQ] = useState("");
  const [filterOpen, setFilterOpen] = useState(false);
  const filterTriggerRef = useRef(null);
  const [show, setShow] = useState({ ok: true, weak: true, section: true, blocked: true, wrongPage: true });
