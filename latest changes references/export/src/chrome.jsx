...
const { useState: useStateChrome, useRef: useRefChrome, useLayoutEffect } = React;

// Portal-based menu so dropdowns never get clipped by panel-body / header overflow.
// Renders into document.body and pins to the bottom-right of the trigger element.
function PortalMenu({ triggerRef, open, onClose, width = 220, children }) {
  const [pos, setPos] = useStateChrome({ top: -9999, left: -9999 });
  useLayoutEffect(() => {
    if (!open || !triggerRef.current) return;
    const r = triggerRef.current.getBoundingClientRect();
    // place below the trigger, aligned to its right edge
...
function Header({ status, dock, setDock, collapsed, setCollapsed, tokenInfo, runState,
  ...
  mode = "llm", setMode = () => {} }) {
  const [dockMenu, setDockMenu] = useStateChrome(false);
  const dockTriggerRef = useRefChrome(null);
    ...
    <header className="aw-header">
      <div className="aw-header-main">
        ...
        <button className="aw-icon-btn"
                onClick={() => {
                  // Open Tweaks panel directly so settings is always functional,
                  // even when not embedded in a host environment.
