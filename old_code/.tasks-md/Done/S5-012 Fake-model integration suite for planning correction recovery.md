...
Implemented:
- `tests/fake_llm_factory.py`: FakeLLMClient with async OpenAI-compatible interface, call capture, configurable purpose_responses, force_malformed mode, cached_tokens support
- Default stubs for: step_plan_normalizer, plan_diff_editor, recovery_diagnoser, page_intelligence_summarizer, locator_specialist

Tests added:
- `tests/test_fake_llm_factory.py`: 18 tests covering call capture, schema-valid stubs per purpose, usage/cached_tokens, malformed output, custom responses, structural invariants

Commands run:
- `python -m pytest tests/test_fake_llm_factory.py -q` → 18 passed

Results:
- 18/18 tests pass
- Full suite: 647 tests pass (up from 604)

Changed files:
- tests/fake_llm_factory.py (new)
- tests/test_fake_llm_factory.py (new)
