...
class LLMRuntimeController:
    ...
    def __init__(
        ...
        *,
        ...
        # Token-report heartbeat callback — set via set_token_report_callback().
        self._token_report_callback: callable | None = None
        self._token_report_interval_calls: int = 3
        self._call_count_since_last_report: int = 0
    ...
    def set_token_report_callback(
        self,
        cb: callable,
        *,
        interval_calls: int = 3,
    ) -> None:
        """Register a heartbeat callback invoked every *interval_calls* LLM calls.

        The callback receives a single dict argument (the result of
        ``get_token_report()``) and its return value is ignored.  The call is
        synchronous and best-effort: any exception raised by *cb* is silently
        swallowed so it never disrupts the controller's normal flow.

        Parameters
        ----------
        cb:
            Callable with signature ``(report: dict) -> Any``.
        interval_calls:
            How many completed LLM calls between successive callback
            invocations.  Must be >= 1; defaults to 3.
        """
        self._token_report_callback = cb
        self._token_report_interval_calls = max(1, int(interval_calls))
        self._call_count_since_last_report = 0
    ...
    async def call(
        ...
        **kwargs: Any,
        ...
        for attempt_index in range(max_attempts):
            ...
            model_called = True
            # Token-report heartbeat: fire callback every N calls (best-effort).
            try:
                self._call_count_since_last_report += 1
                if (
                    self._token_report_callback is not None
                    and self._call_count_since_last_report >= self._token_report_interval_calls
                ):
                    self._token_report_callback(self.get_token_report())
                    self._call_count_since_last_report = 0
            except Exception:  # noqa: BLE001
                pass  # heartbeat must never surface errors to callers
