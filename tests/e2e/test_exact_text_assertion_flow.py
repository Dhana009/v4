...
COMMAND_TEXT = "npx playwright init-agents --loop=opencode"


def test_exact_text_assertion_flow() -> None:
    try:
        from playwright.async_api import async_playwright  # noqa: F401
    except ImportError:
        pytest.skip("playwright.async_api is not available in this environment")

    asyncio.run(_run_exact_text_assertion_flow())


async def _run_exact_text_assertion_flow() -> None:
    app_root = Path(__file__).resolve().parent / "fixtures" / "test_app"
    async with start_e2e_session(test_name="exact_text_assertion_flow", app_root=app_root) as session:
        page = session.page
        ...
        await session.run_stage("exact_text_picker_armed", 10.0, arm_picker)

        async def pick_code_block() -> None:
            await page.get_by_text(COMMAND_TEXT, exact=True).click()
            attach_button = page.get_by_role("button", name="Attach Element").first
            await attach_button.wait_for(state="visible", timeout=8000)
            target_summary = page.locator(".ide-step-target-summary").first
            await target_summary.wait_for(state="visible", timeout=8000)

        await session.run_stage("exact_text_element_picked", 10.0, pick_code_block)

        async def prepare_pending_step() -> None:
            intent_input = page.locator(".ide-step-input").first
            await intent_input.fill(f"assert exact text equal to {COMMAND_TEXT}")
            await page.locator(".ide-step-topline .ide-badge.b-ready").first.wait_for(state="visible", timeout=8000)

        await session.run_stage("exact_text_pending_step_added", 10.0, prepare_pending_step)
        ...
        await session.run_stage("exact_text_run_clicked", 10.0, click_run)
        ...
        await session.run_stage("exact_text_plan_ready_seen", 25.0, wait_for_plan_ready)
        ...
        await session.run_stage("exact_text_confirm_clicked", 10.0, confirm_plan)
        ...
        await session.run_stage("exact_text_execution_started", 15.0, wait_for_execution_started)
        ...
        async def wait_for_recorded_step() -> None:
            ...
            assert "text" in recorded_step_text
            assert "click" not in recorded_step_text
            assert "visible" not in recorded_step_text
            recorded_child_desc = (await recorded_step.locator(".ide-plan-child-desc").first.inner_text()).strip()
            assert COMMAND_TEXT in recorded_child_desc
            assert "expected_outcome" not in recorded_child_desc.lower()
        ...
        await session.run_stage("exact_text_step_recorded_seen", 25.0, wait_for_recorded_step)
        ...
        async def wait_for_code_update() -> None:
            ...
            assert COMMAND_TEXT in recording_line
            assert ".click();" not in recording_line
            assert "toBeVisible()" not in recording_line
            assert ("toContainText(" in recording_line) or ("toHaveText(" in recording_line) or ("has_text" in recording_line)
            assert "expected_outcome" not in recording_line.lower()
            assert page.url == docs_url

        await session.run_stage("exact_text_code_update_seen", 25.0, wait_for_code_update)
